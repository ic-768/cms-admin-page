		...
		<div className="formContainer">
			...
			<div className={"form__content--new"} >
				...
				<input maxLength="50" className={"form__input--text"} placeholder={"Title (max 50 chars)"} value={newPage.title}
					onChange={(event) => {setNewPage({ ...newPage, title: event.target.value })}}/> {/*update title*/}
				...
				<div style={{width:"100%"}}>
					<textarea maxLength="200" className={ "form__input--text" } placeholder={ "Description (max 200 chars)" }
						...
						onChange={(event) => {setNewPage({...newPage, description: event.target.value})}} />{/*update description*/}
				</div>
					...
					<form className={ "radioForm" } >
						<div className="radioContainer" >
							<input type="radio" value="0" id="Menu"
								onChange={ (e) => {setNewPage({ ...newPage, type: parseInt(e.target.value)})}} name="Type" />{/*update type*/} 
				...
				<Calendar value={newPage.publishedOn} onChange={(date)=>{setNewPage({...newPage,publishedOn:date})}}/>{/*directly changes newPage*/}
